...
class Trends extends CI_Controller {
	...
	public function index() {
		...
		$legend = array();
		$values = array();
		$query = $this->db->query('SELECT SUBSTRING(hst.hst_datecreated, 1, 7) AS ref, COUNT(DISTINCT(hst.itm_id)) AS nb FROM '.$this->db->dbprefix('history').' AS hst WHERE hst.hst_datecreated >= ? AND hst.mbr_id = ? GROUP BY ref ORDER BY nb DESC LIMIT 0,12', array($date_ref, $this->member->mbr_id));
		if($query->num_rows() > 0) {
			foreach($query->result() as $row) {
				$legend[] = $row->ref;
				$values[] = $row->nb;
